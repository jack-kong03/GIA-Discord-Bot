...
async def send_morning_routine(channel):
    await send_message(channel, "🌅 Morning Routine: Start your day with a positive mindset and set your intentions.")

async def send_focus_session_1(channel):
    await send_message(channel, "💻 Focus Session 1: Deep work time — tackle your most important tasks.")

async def send_lunch_break(channel):
    await send_message(channel, "🍽️ Lunch Break: Take a well-deserved break and refuel.")

async def send_focus_session_2(channel):
    await send_message(channel, "💻 Focus Session 2: Continue your productive work — keep the momentum going.")

async def send_gym_prep(channel):
    await send_message(channel, "🏋️‍♂️ Gym Prep: Get ready for your workout — hydrate and warm up.")

async def send_gym_session(channel):
    await send_message(channel, "💪 Gym Session: Time to get moving and strengthen your body.")

async def send_evening_routine(channel):
    await send_message(channel, "🌇 Evening Routine: Wind down your work and prepare for the evening.")

async def send_focus_session_3(channel):
    await send_message(channel, "💻 Focus Session 3: Final focused work session — wrap up tasks or plan ahead.")

async def send_dinner_break(channel):
    await send_message(channel, "🍽️ Dinner Break: Enjoy a relaxing dinner and recharge.")

async def send_night_gym_option(channel):
    await send_message(channel, "🏃‍♂️ Night Gym Option: If you feel up to it, a light workout or stretch before bed.")

async def send_relax_session(channel):
    await send_message(channel, "🎮 Relax Session: Time to chill and unwind — enjoy your evening.")

async def send_bed_prep(channel):
    await send_message(channel, "🛏️ Bed Prep: Prepare for sleep — reduce screen time and relax.")

async def send_sleep_reminder(channel):
    await send_message(channel, "😴 Sleep Reminder: It's late — consider getting some rest for a fresh tomorrow.")

async def send_weekend_morning(channel):
    await send_message(channel, "☀️ Weekend Morning: Take it easy and enjoy a leisurely start.")

async def send_weekend_project(channel):
    await send_message(channel, "📚 Weekend Project: Spend some time on personal projects or hobbies.")

async def send_weekend_gym(channel):
    await send_message(channel, "🏋️‍♀️ Weekend Gym: Keep active with a fun workout session.")

async def send_weekend_relax(channel):
    await send_message(channel, "🛋️ Weekend Relax: Rest and recharge — enjoy your weekend downtime.")
...
async def on_ready():
    ...
    if not scheduler.running:
        ...
        scheduler.add_job(send_morning_routine, "cron", day_of_week="0-4", hour=10, minute=0, args=[channel])
        scheduler.add_job(send_focus_session_1, "cron", day_of_week="0-4", hour=10, minute=30, args=[channel])
        scheduler.add_job(send_lunch_break, "cron", day_of_week="0-4", hour=12, minute=0, args=[channel])
        ...
        scheduler.add_job(send_weekend_gym, "cron", day_of_week="5-6", hour=15, minute=0, args=[channel])
        scheduler.add_job(send_weekend_relax, "cron", day_of_week="5-6", hour=17, minute=0, args=[channel])
...
async def show_schedule(ctx):
    ...
    schedule_text = (
        ...
        "10:00 AM — Morning Routine\n"
        "10:30 AM — Focus Session 1\n"
...
async def motivate(ctx):
    ...
    await ctx.send(quotes[datetime.now().day % len(quotes)])
